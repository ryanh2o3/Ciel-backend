...
impl StoryService {
    ...
    pub async fn get_user_stories(&self, user_id: Uuid, viewer_id: Uuid) -> Result<Vec<Story>> {
    ...
    pub async fn add_reaction(
        ...
        if is_new {
            sqlx::query("UPDATE stories SET reaction_count = reaction_count + 1 WHERE id = $1")
                .bind(story_id)
                .execute(self.db.pool())
                .await?;
        }
    ...
    pub async fn list_reactions(&self, story_id: Uuid) -> Result<Vec<StoryReaction>> {
        let rows = sqlx::query(
    ...
    pub async fn list_viewers(&self, story_id: Uuid) -> Result<Vec<StoryView>> {
        let rows = sqlx::query(
    ...
    pub async fn get_user_highlights(&self, user_id: Uuid) -> Result<Vec<StoryHighlight>> {
        let rows = sqlx::query(
    ...
    pub async fn get_stories_feed(&self, user_id: Uuid) -> Result<Vec<Story>> {
        let cache_key = format!("feed:stories:{}", user_id);
